...
--
-- Stand-in structure for view `d10_v8`
...
--
-- Stand-in structure for view `d10_v11`
-- (See below for the actual view)
--
CREATE TABLE `d10_v11` (
`Code` int(3)
,`Name` varchar(255)
,`Price` decimal(10,0)
,`Manufacturer` int(3)
...
--
-- Structure for view `d10_v8`
--
DROP TABLE IF EXISTS `d10_v8`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `d10_v8`  AS  select `products`.`Code` AS `code`,`products`.`Name` AS `product`,`manu`.`Name` AS `manufacturer` from (`manu` join `products` on((`products`.`Code` = `manu`.`Code`))) ;

-- --------------------------------------------------------

--
-- Structure for view `d10_v11`
--
DROP TABLE IF EXISTS `d10_v11`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `d10_v11`  AS  select `products`.`Code` AS `Code`,`products`.`Name` AS `Name`,`products`.`Price` AS `Price`,`products`.`Manufacturer` AS `Manufacturer` from `products` ;
...
--
ALTER TABLE `products`
  MODIFY `Code` int(3) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=27;
--
