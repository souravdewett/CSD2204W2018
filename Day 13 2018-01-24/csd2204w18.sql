...
--
-- Host: 127.0.0.1
-- Generation Time: Jan 24, 2018 at 05:25 PM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `job_search` (IN `salary` INT)  BEGIN SELECT * from jobs where minsalary>salary;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `job_search1` (IN `salary` INT)  BEGIN SELECT * from jobs where min_salary>salary; END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `job_search2` (IN `SALARY` INT, OUT `CNTEMP` INT)  BEGIN select count(*) INTO CNTEMP from jobs where MAX_salary >salary;
END$$
...
DECLARE a,b INT;
DECLARE cur_1 CURSOR FOR SELECT max_salary FROM jobs; DECLARE CONTINUE HANDLER FOR NOT FOUND SET b = 1; OPEN cur_1;
REPEAT
FETCH cur_1 INTO a; UNTIL b = 1
END REPEAT; CLOSE cur_1;
SET return_val = a;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `proc_INOUT1` (INOUT `maxsalary` INT, IN `desig` CHAR(30))  BEGIN SELECT MAX_SALARY INTO maxsalary FROM jobs WHERE JOB_TITLE = desig; END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `proc_Uer_Variables` ()  BEGIN
SET @x = 15;
SET @y = 10;
SELECT @x,@y,@x-@y;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `proc_WHILE1` (IN `n` INT)  BEGIN
SET @sum = 0; SET @x = 1; WHILE @x<n DO
IF mod(@x, 2) <> 0
THEN SET @sum = @sum + @x; END IF;
SET @x = @x + 1;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_Airthematic_Operation` (`caseValue` VARCHAR(10), `valueA` INTEGER(10), `valueB` INTEGER(10))  BEGIN
CASE caseValue
WHEN "Add" THEN SELECT (valueA+valueB) as Addition; WHEN "Sub" THEN SELECT (valueA-valueB) as Subtraction;
WHEN "Mul" THEN SELECT (valueA*valueB) as Multiple; WHEN "Div" THEN SELECT (valueA/valueB) as Divide; ELSE
BEGIN
END; END CASE;
END$$
...
INSERT INTO test.customer(customer_id,cust_name) VALUES(3001,"test");
SELECT *FROM test.customer; END$$
...
--
DELIMITER $$
CREATE TRIGGER `class_activity_UP` BEFORE UPDATE ON `custom` FOR EACH ROW BEGIN
